...
# AUDIT STATUS: Pre-Promote Raw Build (GZIP Decompression Fix)
# ==============================================================================
...
LAKE_NAME = "b5_pdb_protein"

# RCSB PDB APIs
SEARCH_API_URL = "https://search.rcsb.org/rcsbsearch/v2/query"
# FIX: PDB validation reports are gzipped (.xml.gz)
...
# Sample limit for Litmus test
STRUCTURE_LIMIT = 1500

def get_high_res_pdb_ids():
    """Queries PDB for X-ray structures with resolution <= 2.5A"""
    print(f"  -> Querying PDB Search API for high-resolution structures...")
    query = {
        "query": {
            "type": "terminal",
            "service": "text",
        ...
        "request_options": {
            "paginate": {"start": 0, "rows": STRUCTURE_LIMIT}
        },
        "return_type": "entry"
    }
...
def build():
    print(f"\n[{LAKE_NAME.upper()}] Initiating extraction from RCSB PDB...")
    
    try:
        pdb_ids = get_high_res_pdb_ids()
        print(f"  -> Retrieved {len(pdb_ids)} PDB IDs for litmus sample.")
