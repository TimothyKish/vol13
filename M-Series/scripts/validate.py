# ==============================================================================
# SCRIPT: validate.py
# PURPOSE: Trustless Auditor for the Master Materials promoted JSONL.
# ==============================================================================
import json
from pathlib import Path

LAKE_ID = "materials"
PROMOTED_PATH = Path(__file__).resolve().parents[3] / "lakes" / "inputs_promoted" / f"{LAKE_ID}_promoted.jsonl"

def validate():
    print(f"--- VALIDATING: {LAKE_ID} ---")
    if not PROMOTED_PATH.exists(): 
        return print(f"FAIL: {PROMOTED_PATH.name} missing.")
        
    valid_count = 0
    with PROMOTED_PATH.open("r", encoding="utf-8") as f:
        for line in f:
            if not line.strip(): continue
            rec = json.loads(line)
            assert "domain" in rec, "Missing domain"
            assert "entity_id" in rec, "Missing entity_id"
            assert "geometry_payload" in rec, "Missing geometry_payload"
            # Assuming Volume 3 legacy files might not have _raw_payload perfectly mapped yet,
            # we check for the explicit domain assignment
            assert rec.get("domain") == "materials", "Domain mismatch"
            valid_count += 1
            
    print(f"SUCCESS: {valid_count:,} records passed structural validation.")

if __name__ == "__main__": 
    validate()