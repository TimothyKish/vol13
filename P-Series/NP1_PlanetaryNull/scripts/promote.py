# ==============================================================================
# SCRIPT: promote.py
# PURPOSE: Maps synthetic NP1 planetary data to the KishLattice sovereign schema.
# ==============================================================================
import json, uuid
from datetime import datetime, timezone
from pathlib import Path

DOMAIN = "null_planetary"
LAKE_ID = "np1_planetary_null"
TARGET_FIELD = "orbital_period_days"
...
def promote():
    ...
    with RAW_PATH.open("r", encoding="utf-8") as fin, PROMOTED_PATH.open("w", encoding="utf-8") as fout:
        for line in fin:
            ...
            rec = json.loads(line)
            promoted = {
                "entity_id": rec.get("entity_id", str(uuid.uuid4())), 
                "domain": DOMAIN, "volume": 11, "lake_id": LAKE_ID,
                TARGET_FIELD: rec["pl_orbper"], 
                "geometry_payload": {"coordinates": [], "dimensionality": 1, "geometry_type": "scalar"},
                "meta": {"source": "Synthetic Null Engine", "timestamp": now, "sovereign": True},
                "_raw_payload": rec
            }
            fout.write(json.dumps(promoted) + "\n"); count += 1
            
    print(f"SUCCESS: {count:,} synthetic planetary orbits promoted.")
