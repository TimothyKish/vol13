# ==============================================================================
# SCRIPT: validate.py (P4_TTV)
# TARGET: Litmus gate for Kepler transit timing variation lake.
...
#   Computed on RAW ttv_absolute_minutes values (in minutes).
#   Matches the Vol 9 validation standard applied to all prior lakes.
#
# EXPECTED BEHAVIOUR:
#   The TTV distribution is heavily right-skewed. Most planet transits deviate
#   by only a few minutes from their Keplerian timing. A small fraction deviate
#   by hundreds of minutes (strongly perturbed multi-planet systems).
#   The span is anchored at 1440.0 minutes (the build exclusion limit).
#   This produces a very low stdev/span ratio — expected and not pathological.
#   The chaos null is built from the same distribution and controls for this.
# ==============================================================================
...
def validate():
    ...
    print(f"    Min:                  {v_min:.6f} minutes")
    print(f"    Max:                  {v_max:.6f} minutes")
    print(f"    Span:                 {v_span:.6f} minutes")
    print(f"    Mean:                 {v_mean:.6f} minutes")
    print(f"    StDev:                {v_std:.6f} minutes")
    print(f"    StDev / Span:         {ratio:.4f}")
    print()

    if ratio < LITMUS_LIMIT:
        print(f"  LITMUS: PASS ({ratio:.4f} < {LITMUS_LIMIT})")
        print(f"  Note: Low ratio expected for right-skewed TTV distribution.")
        print(f"  Chaos null controls for non-uniform distribution.")
