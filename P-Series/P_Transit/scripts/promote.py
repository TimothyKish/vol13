...
RAW_PATH = SCRIPT_DIR.parent / "lake" / "p_transit_raw.jsonl"
PROMOTED_DIR = SCRIPT_DIR.parents[2] / "lakes" / "inputs_promoted"
PROMOTED_PATH = PROMOTED_DIR / "p_transit_promoted.jsonl"

def promote():
    print("=" * 60)
    print(" P_TRANSIT PROMOTION SCRIPT ".center(60))
    print("=" * 60)
    ...
    with RAW_PATH.open("r", encoding="utf-8") as fin, PROMOTED_PATH.open("w", encoding="utf-8") as fout:
        for line in fin:
            ...
            raw_val = data_source.get("pl_trandur")
            if raw_val is not None and str(raw_val).strip() != "":
                try:
                    ...
                    promoted_rec = {
                        ...
                        "domain": "orbital_transit",
                        "volume": 11,
                        "lake_id": "p_transit",
                        "transit_duration_hr": clean_val,
                        "geometry_payload": {"coordinates": [], "dimensionality": 1, "geometry_type": "scalar"},
                        "meta": {"source": "NASA Exoplanet Archive (Transit Duration)", "ingest_timestamp": now_ts, "sovereign": True},
