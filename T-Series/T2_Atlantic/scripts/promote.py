...
PROMOTED_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "..", "lakes",
                              "inputs_promoted", "t2b_atlantic_promoted.jsonl")
...
def promote():
    ...
    with open(RAW_PATH, encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            records.append(r)
    if errors:
        print("[T2b_Atlantic] PROMOTE FAILED:"); [print(f"  {e}") for e in errors[:10]]; sys.exit(1)
    with open(PROMOTED_PATH, "w", encoding="utf-8") as f:
        for r in records:
            r["promoted"] = True; r["lake_version"] = "vol9"
            f.write(json.dumps(r) + "\n")
    print(f"[T2b_Atlantic] promote complete — {len(records)} records — 0 errors")
