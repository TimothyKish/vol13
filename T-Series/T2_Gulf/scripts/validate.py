...
def validate():
    ...
    for i, r in enumerate(records):
        ...
        if r.get("domain") != "planetary_gulf": errors.append(f"row {i+1}: domain mismatch")
    if errors:
        print("[T2c_Gulf] VALIDATE FAILED:")
        [print(f"  {e}") for e in errors[:5]]; sys.exit(1)
    scalars = [r["kish_scalar"] for r in records]
    target_19 = 19/PI  # T2c confirmed at 19/pi
    near_19 = sum(1 for r in records if abs(r["n_approx"] - 19) < 0.5)
    print(f"[T2c_Gulf] VALIDATE PASSED")
    ...
    print(f"  [READY FOR PIPELINE] domain: planetary_gulf")
