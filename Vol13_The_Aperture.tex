...
\begin{titlepage}
    ...
    {\normalsize Volume 13 DOI: \url{https://doi.org/10.5281/zenodo.22851850} \par}
    \vspace{0.3cm}
...
% ==============================================================================
\chapter{Deriving $f$: From Named Parameter to Four-Dimensional Potential}
% ==============================================================================

\begin{center}
\textit{The audit of the theory (Chapter~\ref{chap:hunt-f}) found the central parameter of the
Lattice Energy expression undefined. This chapter derives it, at the cost of one postulate the
framework already half-states, and reports the four errata the derivation produced along the way.
It is presented in full because the programme's standard is that meaning must be hammered to
mathematics, not described and left inferred.}
\end{center}

\section{The Parameter That Was Never Defined}

The Rosetta Atlas states the Lattice Energy of a mass $m$ at radius $r$ as
...
and defines $f$, in full, as: \emph{``the slipstream factor (drag-corrected motion through the
lattice).''} No formula, no dimensions, no construction. A dimensional audit shows the expression
is an energy only if $f$ carries $[L^4][T^{-2}]$ --- but ``slipstream,'' everywhere else in the
corpus, denotes a \emph{dimensionless} damping factor on $[0,1]$. Three incompatible readings were
in circulation: $f$ as a dimensionless drag factor (in which case $E_{\mathrm{lat}}$ is not an
energy), $f$ as a dimensional carrier (in which case it has no connection to the drag physics it is
named for), and $f$ as a stand-in for the gravitational constant (in which case the pure number
$16/\pi$ cannot be replacing the dimensional $G$).

\begin{formbox}
An undefined load-bearing parameter is the theory-side twin of the empirical programme's worst
failure mode: a quantity that absorbs whatever the equation requires. Volume~12 withdrew a headline
domain for the empirical version of exactly this. The theory could not be held to a lower standard
than the instrument.
\end{formbox}

\section{The One Postulate That Fixes It}

Dimensional analysis alone cannot choose the radial power: since $\ell_{\mathrm{cell}}/r$ is
dimensionless, $f = \ell_{\mathrm{cell}}^2 c^2 (\ell_{\mathrm{cell}}/r)^{n}$ satisfies the dimensions
for \emph{every} $n$. An additional principle is required, and the framework already contains it.

\begin{signalbox}
\textbf{Postulate (already implicit in Volume~1).} The lattice bulk is four-dimensional in space.
Volume~1 states it directly --- ``mapping a 4D hypercube onto a cyclic time loop'' --- it is embodied
in the 24-cell, a four-dimensional regular polytope, and it is already used in
$N_{\mathrm{total}} = d^2 = 4^2 = 16$, the mode count that produces the modulus itself.
\end{signalbox}

Made explicit, this postulate determines the radial power by Gauss's law.

\subsection{Gauss's law in four spatial dimensions}

\begin{derivbox}
For a point source of strength $Q$ in $d$ spatial dimensions, flux conservation through the
enclosing $(d{-}1)$-sphere gives
\[
...
\begin{equation}
V(r) = -\!\int E\, dr = \frac{Q}{(d-2)\,\Omega_{d-1}}\cdot\frac{1}{r^{\,d-2}} \qquad (d \neq 2).
\label{eq:gauss-d}
\end{equation}

\medskip
For $d = 4$, with $\Omega_3 = 2\pi^2$:
\[
V(r) = \frac{Q}{2\cdot 2\pi^2}\cdot\frac{1}{r^2} = \frac{Q}{4\pi^2\, r^2}.
\]
\textbf{The potential of a point source in four spatial dimensions falls as $1/r^2$} --- exactly the
form of Equation~\ref{eq:elat-f}, and $n = 0$ is fixed, not assumed.
\end{derivbox}

\subsection{The coupling}

With the source strength set by the lattice scale, $Q = \kappa\, m\, \ell_{\mathrm{cell}}^2 c^2$ for
a dimensionless $\kappa$, the interaction energy of a test mass is

\begin{equation}
E_{\mathrm{lat}} = m\, V(r) = \frac{\kappa}{4\pi^2}\; m c^2 \left(\frac{\ell_{\mathrm{cell}}}{r}\right)^2 .
\end{equation}

Matching to Equation~\ref{eq:elat-f} requires

\begin{equation}
\frac{\kappa}{4\pi^2} = \frac{16}{\pi}
\quad\Longrightarrow\quad
\boxed{\;\kappa = 64\pi = 4\,N_{\mathrm{total}}\,\Phi_{\mathrm{cycle}}\;}
\end{equation}

--- four times the metric-component count times the cyclic phase. Whether that factorisation is
physically meaningful or a numerical accident is left open; it is recorded so it can be tested
against the geometry by a hand more expert in the 24-cell than this programme's.
...
\begin{tikzpicture}[scale=1.0]
  \begin{scope}
    ...
    \foreach \a in {0,45,...,315} \draw[->, kishblue!55] (0,0) -- (\a:1.3);
    \draw[gray] (0,0) circle (1.3);
    \node at (0,-1.9) {\small $d=3$: $V \sim 1/r$};
    \node[gray] at (0,-2.35) {\footnotesize flux over $r^2$};
  \end{scope}
  \draw[->, very thick] (2.0,0) -- (3.4,0);
  \node[above] at (2.7,0.08) {\footnotesize one more};
  \node[below] at (2.7,-0.08) {\footnotesize dimension};
  \begin{scope}[xshift=5.4cm]
    \fill[cruciblered] (0,0) circle (2.5pt);
    \foreach \a in {0,30,...,330} \draw[->, cruciblered!55] (0,0) -- (\a:1.3);
    \draw[gray] (0,0) circle (1.3);
    \draw[gray, dashed] (0,0) ellipse (1.3 and 0.45);
    \node at (0,-1.9) {\small $d=4$: $V \sim 1/r^2$};
    \node[cruciblered] at (0,-2.35) {\footnotesize flux over $r^3$};
  \end{scope}
\end{tikzpicture}
\caption{The radial power, derived. A point source spreads its flux over the enclosing
$(d{-}1)$-sphere, so its potential falls as $1/r^{d-2}$. In four spatial dimensions this is $1/r^2$
--- the Atlas form. With it, $f = \ell_{\mathrm{cell}}^2 c^2$ and $\kappa = 64\pi$ follow without any
further adjustment.}
\end{figure}

\section{What the Derivation Costs}

The derivation is honest only if its price is stated as plainly as its result.

\begin{warningbox}
We observe \emph{three} spatial dimensions, and both gravity and electrostatics fall as $1/r$, not
$1/r^2$. \textbf{A $1/r^2$ lattice energy therefore cannot be the interaction measured at ordinary
scales.} It can only be a bulk quantity whose projection onto the three-dimensional boundary
recovers $1/r$ --- and that projection is computed nowhere in the corpus.

\medskip
Volume~1's holographic language (``a holographic projection on the boundary preserves the full
information content of the bulk tensor'') gestures at it. It does not perform it.
\end{warningbox}

\begin{signalbox}
The framework has therefore traded a \emph{vague} gap for a \emph{precise} one. Before: an undefined
$f$ and an unfixed radial power. After: a derived $f = \ell_{\mathrm{cell}}^2 c^2$, a derived
$n = 0$, a derived coupling $\kappa = 64\pi$, and a single well-posed open problem --- the
\textbf{bulk-to-boundary projection} that would turn a four-dimensional $1/r^2$ into the
three-dimensional $1/r$ we measure. That is progress: a question a competent theorist can attack,
in place of a parameter that meant whatever it needed to.
\end{signalbox}

\section{$\ell_{\mathrm{cell}}$ Is a Postulate, Not a Result}

One further honesty is required. Every input to the modulus derivation is dimensionless: the mode
count $N_{\mathrm{total}} = 16$, the cyclic phase $\Phi_{\mathrm{cycle}} = \pi$, the 24-cell packing.
\textbf{No length can be derived from dimensionless inputs.} The cell spacing
$\ell_{\mathrm{cell}}$ is therefore imported, not derived --- Volume~1 sets it to the Planck length,
which brings $\hbar$, $G$, and $c$ in from outside the framework.

This is not a defect peculiar to KishLattice; every discrete-spacetime programme faces it. But it
must be \emph{stated as a postulate}, and the theory documents did not: they variously set
$\ell_{\mathrm{cell}} = \ell_P$ (Volume~1) and implied $\ell_{\mathrm{cell}} \approx 3.9\times10^{7}$~m
via a laboratory-frequency chain (Rosetta Atlas), a disagreement of \emph{forty-two orders of
magnitude}. The Atlas chain is unit-dependent and is withdrawn (below); the Planck identification
stands, as an explicit postulate.

\section{The Four Errata the Derivation Produced}

An audit that yields only its headline and not the errors it exposed is not an audit.

\begin{erratumbox}
\textbf{Erratum V1--1 (Volume~1, \S1.4).} The claim that $16/\pi$ possesses ``the physical
dimensions of Metric Tension per Unit Volume'' is withdrawn. Equation~1.3 derives
$16/\pi = N_{\mathrm{total}}/\Phi_{\mathrm{cycle}}$ as a ratio of a mode count to a phase angle ---
\emph{dimensionless}. The relation $H_{\mathrm{local}} = H_{\mathrm{early}} + 16/\pi$ built on the
dimensional claim holds only in km\,s$^{-1}$\,Mpc$^{-1}$ ($67.4 + 5.09 = 72.5$ against SH0ES $73.0$)
and fails by eighteen orders of magnitude in SI. It is a unit coincidence, and is withdrawn.
\end{erratumbox}

\begin{erratumbox}
\textbf{Erratum V1--2 (Volume~1, Listing~14.1).} The speed-of-light verification computes
$c_{\mathrm{mech}} = \ell_P f_P / (16/\pi)$. But $\ell_P f_P = c$ by the definition of Planck units,
so dividing by the modulus yields $58{,}864{,}091$~m/s, not $c$. The script prints this, then a
hardcoded line reading the observed $c$, then ``5-SIGMA ALIGNMENT LOCKED'' --- a print statement, not
a comparison. The listing is withdrawn. Separately, $c = \ell_P f_P$ is an identity of Planck units,
not a derivation.
\end{erratumbox}

\begin{erratumbox}
\textbf{Erratum RA--1 (Rosetta Atlas).} The ``mirrors Newton's $F = Gm/r^2$'' passage is struck.
$Gm/r^2$ is a field with the dimensions of acceleration, compared against an energy; gravitational
potential energy goes as $1/r$; and $G$ carries dimensions while $16/\pi$ is a pure number, making
the stated substitution dimensionally impossible. The expression is a four-dimensional point-source
potential, not a gravitational analogue.
\end{erratumbox}

\begin{erratumbox}
\textbf{Erratum RA--2 (Rosetta Atlas).} The chain $c = \ell_{\mathrm{cell}} f_{24}$ with
$f_{24} = 24/\pi$~Hz, implying $\ell_{\mathrm{cell}} \approx 3.9\times10^{7}$~m, is withdrawn. It
privileges the second --- a caesium-hyperfine human convention --- and is therefore unit-dependent,
the theory-side form of the empirical programme's unit-dependence erratum. $\ell_{\mathrm{cell}}$ is
set to the Planck length by postulate instead.
\end{erratumbox}

\section{What This Chapter Settles, and What It Does Not}

\begin{signalbox}
\textbf{Settled.} $f = \ell_{\mathrm{cell}}^2 c^2$, derived from four-dimensional Gauss's law. The
radial power $n = 0$, derived. The coupling $\kappa = 64\pi$, derived. Four published theory claims,
withdrawn as errata. The slipstream name reattached to a separate, phenomenological drag factor
$F(d,T)$ rather than to $f$ itself.

\medskip
\textbf{Not settled.} The bulk-to-boundary projection --- the framework's central open problem, on
which its entire connection to observed three-dimensional physics now rests. And the value of
$\ell_{\mathrm{cell}}$, which is a postulate, not a result, in this framework as in every other
discrete-spacetime theory.
\end{signalbox}

The parameter that meant whatever the equation required now has a derived form and one honestly
named gap. That is a smaller claim than a completed theory of gravity, and a far more defensible one.
It is the difference between a framework that describes its meaning and one that hammers it to
mathematics --- and it is the standard the rest of this programme is held to.
...
% ==============================================================================
\chapter{The Strain Survey: Four Honest Runs Under Strict I/O}
% ==============================================================================

\begin{center}
\textit{The strain formulation of Chapters~5--9 is machinery. This chapter is what the machinery
produced when finally run against real lakes under the strict input contract --- the contract
adopted after a fallback defect was found to substitute calculated values for missing data and
report the substitution as agreement. Every run here either reads a real lake or aborts by name.
None simulates. The results are modest, two are aborts, one is a null against the framework, and
one is a located data defect. All four are reported exactly as the engines returned them.}
\end{center}

\section{The Contract These Runs Enforce}

Each kinematic engine reads one promoted lake, resolves the physical field it requires, and either
computes or \emph{aborts with the field named}. There is no path by which a missing field yields a
number. The abort message dumps every key actually present in the lake, lists the candidate field
names tried, and prints the sentence that matters: \texttt{No values were simulated}. This is the
operational form of the lesson from the fallback defect --- an engine that cannot find its data must
say so, never fill the gap.

Two of the four runs abort under this contract. That is not a failure of the runs; it is the
contract working, and the aborts are as much a result as the computations.

\section{Planetary --- A Clean Abort}

\texttt{build\_planetary\_kinematic\_engine.py} requires host-star mass to compute orbital lattice
strain. The lake it reads, \texttt{p2\_orbital\_radius} (NASA Exoplanet Archive), carries orbital
semi-major axis and nothing else:

\begin{lstlisting}
  ABORT -- required field 'm_star_sun' not present in p2_orbital_radius
  Tried: st_mass, stellar_mass_sun, host_mass_msun, m_star_solar, st_mass_msun
  Keys present: ... pl_orbsmax = 0.0175 ... (no stellar mass anywhere)
  No values were simulated. Fix the lake or the field mapping and re-run.
\end{lstlisting}

\begin{nullbox}
\textbf{Result: aborted, field absent.} The planetary strain test cannot run until a lake carrying
host-star mass is promoted. The abort is clean, named, and simulates nothing. This is the strict
I/O contract doing precisely its job --- the same class of missing-field condition that, under the
earlier fallback, would have silently produced calculated ``agreement.''
\end{nullbox}

\section{Cosmological --- An Abort, With the MOND Scale Derived First}

\texttt{build\_cosmological\_kinematic\_engine.py} aborts identically --- \texttt{g1\_galaxy\_kinematics}
carries velocity dispersion but no baryonic mass. But before reaching the missing field, it computes
the MOND acceleration scale from first principles, and that computation is a genuine, if modest,
result:

\begin{signalbox}
\[
a_0^{\text{derived}} = \frac{cH_0}{2\pi} = 1.0422\times10^{-10}~\mathrm{m/s^2}
\quad(H_0 = 67.4~\mathrm{km\,s^{-1}\,Mpc^{-1}})
\]
\[
a_0^{\text{MOND fit}} = 1.2000\times10^{-10}~\mathrm{m/s^2},
\qquad \frac{\text{derived}}{\text{fitted}} = 0.8685 \;\; (-13.2\%)
\]
\end{signalbox}

The derived value lands 13\% below the empirical MOND scale. The engine states the discipline
itself: \emph{``The DERIVED value is the framework's prediction. Scoring with the fitted value would
be testing MOND against MOND.''}

\begin{warningbox}
\textbf{Result: a 13\% prediction, not a fit.} A $cH_0/2\pi$ estimate of $a_0$ is a known
order-of-magnitude coincidence in the MOND literature, and $16/\pi$ does not appear in it. The
framework reproduces the \emph{scale} to within 13\% from $c$ and $H_0$ alone --- worth recording,
not worth overselling. The rotation-curve test that would use it aborts for want of baryonic mass in
the lake; a SPARC-derived lake is the prerequisite, and it is named as such rather than worked
around.
\end{warningbox}

\section{Amino Acids --- The Modulus Sweep Says No}

\texttt{build\_amino\_kinematic.py} ran in full: twenty amino-acid molecules, bond lengths grouped
into C--C, C--N single, and C--N aromatic. Rather than ask whether $16/\pi$ fits --- near-hits are
cheap when nodes are 7\% apart --- it ran the test that matters, sweeping 40{,}000 random moduli and
asking how often a randomly chosen modulus does at least as well as $16/\pi$.
...
\begin{tabular}{@{}lrrl@{}}
\toprule
\textbf{Bond} & \textbf{$n$} & \textbf{$16/\pi$ lock @0.05} & \textbf{random moduli $\geq 16/\pi$} \\
\midrule
C--C (sp$^3$) & 68 & 2.9\% & \textbf{57.7\%} \\
C--N single & 23 & 0.0\% & \textbf{100.0\%} \\
C--N aromatic & 11 & 9.1\% & --- (0.9 node cells, cannot resolve) \\
\bottomrule
\end{tabular}
\caption{The amino-acid modulus-sweep null. A randomly chosen modulus matches or beats $16/\pi$ 58\%
of the time on C--C bonds and 100\% of the time on C--N single bonds.}
\end{table}

\begin{signalbox}
\textbf{Result: $16/\pi$ is NOT privileged on amino-acid bond lengths.} The engine's own verdict.
An analytic cross-check confirms the mechanism: nodes spaced 7.0\% apart give landing within 1.5\%
of \emph{some} node a prior probability near 43\%, and the measured p-values ($39$--$58\%$) sit in
that regime. The bond data also fails on independence --- these are repeated measurements of $\sim3$
chemical constants, not $\sim150$ independent samples, and two of the three groups span less than one
node cell, too narrow to resolve a period at all.
\end{signalbox}

This is the first time the modulus itself was tested against alternatives on real data, and it did
not survive. It is recorded here in full because a programme that reports only its confirmations has
told the reader nothing about its error rate. The near-coincidences of $16/\pi$ with physical
observables --- water-bond angles and the like --- must now be read against this null: on the one
dataset where the test was run cleanly, the modulus was ordinary.

\section{Black Holes --- What We Ran, What We Saw, and the Rebuild It Requires}

\texttt{build\_blackhole\_kinematic\_engine.py} ran in full against \texttt{l1\_gwtc} --- 83
gravitational-wave events, 100\% field coverage. For each event it inverted a black-hole spin from
the ringdown frequency \texttt{f\_ring\_hz} and the final mass \texttt{final\_mass\_solar}, and
tested the result against the Kerr bound. What the run saw is stated plainly, because the point of
the strict-I/O rebuild is that we report the run, not a cleaned-up version of it.

\subsection{What we saw}

\begin{erratumbox}
\textbf{81 of 83 events fall below the non-spinning Kerr floor.} The ratio
$f_{\text{obs}}/f_{\text{Kerr}}(a{=}0)$ ranges from 0.144 to 1.399, median 0.731 --- that is, the
median event's stated ringdown frequency is only 73\% of the \emph{lowest} frequency a
non-spinning black hole of that mass could produce. A fundamental $\ell = m = 2$ ringdown cannot
sit below the $a=0$ floor: it is physically impossible, not merely unusual. Only two events
inverted to a real spin; none exceeded the extremal ceiling.
\end{erratumbox}

\subsection{What we determined the cause to be}

We did not leave this as an anomaly. A direct calculation identifies the cause and closes it.
...
\begin{tabular}{@{}lrrrl@{}}
\toprule
\textbf{Event} & \textbf{$M_f$} & \textbf{$f_{\text{ring}}$} & \textbf{$f_{\text{Kerr}}(a{=}0)$} & \textbf{ratio / type} \\
\midrule
GW190425 & 3.4 & 510 & 3552 & 0.14 --- binary neutron star \\
GW200115 & 6.0 & 620 & 2013 & 0.31 --- neutron-star--black-hole \\
GW151012 & 35.7 & 145 & 338 & 0.43 --- \textbf{clean binary black hole} \\
\bottomrule
\end{tabular}
\caption{The worst offenders, checked. The first two are compact-object classes that do not
produce a Kerr ringdown at all and should never have entered a QNM analysis. But the third,
GW151012, is a bona-fide binary black hole and still sits 57\% below its own non-spinning floor.}
\end{table}

\begin{signalbox}
\textbf{The diagnosis is definite.} If the sub-floor events were only the neutron-star cases, the
fix would be a contaminant cut. But a clean binary black hole below the $a=0$ floor is impossible
for a true ringdown --- so \texttt{f\_ring\_hz} is \emph{not} the fundamental $\ell = m = 2$ QNM,
systematically, across the whole catalogue. It is most consistent with the dominant
near-merger gravitational-wave strain frequency (of order twice the orbital frequency), which runs
well below the ringdown mode. The engine's verdict stands, now proven rather than suspected:
\emph{``an instrument or provenance fault is far likelier than a Kerr violation. This is a lake
finding, not a physics one.''}
\end{signalbox}

\subsection{Why we cannot patch it in this volume}

The lake carries exactly one frequency column, \texttt{f\_ring\_hz}, alongside
\texttt{chirp\_mass\_solar} and \texttt{final\_mass\_solar}. There is no \texttt{f\_220},
\texttt{f\_qnm}, or damping time to switch to. \textbf{The correct observable is not present in the
lake at all}, so there is no field-mapping repair --- the strong-field ringdown test cannot be run
correctly against this data, and we do not run it incorrectly to produce a number.

\begin{predictionbox}
\textbf{Carried to Volume~14: the \texttt{l1\_gwtc} rebuild.} The ringdown test is deferred, and
its prerequisite is named exactly. The lake will be rebuilt from a source that publishes the
fundamental quasi-normal mode directly --- the LVK ringdown analyses or the \texttt{pyRing} results,
which report $f_{220}$ and $\tau_{220}$ per event --- with each event's \emph{measured} remnant
mass matched to its \emph{measured} $f_{220}$, binary-neutron-star and neutron-star--black-hole
events excluded by construction, and the required fields (\texttt{f\_220\_hz},
\texttt{tau\_220\_s}, \texttt{final\_mass\_solar}, \texttt{final\_spin}) asserted present at
promotion. Only then does the strong-field yield test of Chapter~8 run on the correct quantity.
The falsification radius must be filed before that run, not after.
\end{predictionbox}

\subsection{The yield radius, and a flag the engine raised on itself}

The run also swept the yield radius $r/r_s = (\pi/32)/\varepsilon_y$, and flagged --- unprompted ---
the trap in its own most elegant number:

\begin{rulingbox}
Since $\varepsilon(r_s) = \pi/32$ exactly, choosing $\varepsilon_y = \pi/32$ places the yield
surface \emph{exactly at the horizon} and every predicted deviation inside it --- elegant, and
unobservable. \textbf{If that value is to be claimed it must be derived from the Lagrangian and the
falsification radius filed before the derivation, not arrived at after noticing it is safe.} An
unfalsifiable coincidence is not a prediction. This flag stands regardless of the lake rebuild:
even with clean $f_{220}$ data, $\varepsilon_y$ must be derived, not chosen.
\end{rulingbox}

\section{What the Strain Survey Established}
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Domain} & \textbf{Outcome} & \textbf{Meaning} \\
\midrule
Planetary & clean abort & no host-star mass in lake; strict I/O working \\
Cosmological & abort, $a_0$ derived & MOND scale to 13\%; rotation test needs SPARC \\
Amino & \textbf{ran; null} & $16/\pi$ not privileged (58--100\% of moduli match it) \\
Black hole & ran; diagnosed & 81/83 sub-Kerr; wrong frequency observable; V14 rebuild named \\
\bottomrule
\end{tabular}
\end{table}

\begin{signalbox}
Two aborts, one null against the framework, one located data defect. Not a single manufactured
agreement, and not a single simulated value. The strain machinery is built; most of its tests cannot
yet run for want of clean lake fields; and the one test that ran cleanly --- the amino-acid modulus
sweep --- found the modulus ordinary.

\medskip
This is the strain program's honest standing. It is a smaller claim than a confirmed theory of
gravity, and it is reported by engines that refuse to lie about it. That refusal, demonstrated four
times over, is the result worth publishing.
\end{signalbox}
...
% ==============================================================================
\chapter{The Period Engine: Operation, Outputs, and Commands}
% ==============================================================================

\begin{center}
\textit{An operator's account of the second 1D engine as it stands: what it computes,
how it is run, what it emits, and how it adapts to the machine it runs on. The engine is
in active development; this chapter documents its current, working form.}
\end{center}

\section{What the Period Engine Is, in One Paragraph}

The phase engine tests \emph{where} on the geometric grid a domain's structure sits. The period
engine tests \emph{whether} the structure repeats with the lattice period at all, using the
circular resultant of $\ln x$ --- and it does so with unit invariance as a proved identity rather
than an empirical hope (the period-engine chapter of Volume 12). It is a separate, immutable
codebase from the phase engine: no shared code, its own configuration, its own 160-character
fingerprint. It reads the same promoted lakes through the same field-resolution contract, and it
computes $\ln x$ and applies the modulus \emph{only at test time}, so its lakes carry raw values
and no baked-in constant. That is what makes a period lake framework-agnostic: a sceptic may take
it and test any modulus, or none.

\section{The Pipeline Stages}

A full run passes through five named stages, familiar in shape to anyone trained on the phase
engine, resumable with \texttt{-{}-from}:
...
\midrule
\texttt{logify}  & \texttt{logify.py} & raw promoted lake $\to$ $\ln x$ lake; Form-C guard \\
\texttt{support} & \texttt{support\_profile.py} & effective-span gate + shape verdict (mandatory) \\
\texttt{scan}    & \texttt{periodogram.py} & resultant excess per register, resolution-gated \\
\texttt{census}  & \texttt{register\_census.py} & survey-wide: which registers resolve \emph{and} lock \\
\texttt{figures} & \texttt{generate\_figures\_period.py} & optional report plugins (Vera) \\
\bottomrule
\end{tabular}
\caption{The period-engine pipeline. The \texttt{support} stage is the one the phase engine never
had, and the one that earns its place: it measures \emph{effective} span, not min--max, so a sparse
tail cannot inflate a domain's apparent resolution.}
\end{table}

\subsection{The support stage, and why it is mandatory}

The resolution criterion of the resolution-criterion chapter is computed on log-span. A domain's
\emph{raw} min--max span can be stretched by a handful of outlier records that carry almost no mass,
making a domain look resolvable when its actual support occupies a narrow band. The support stage
therefore measures the \textbf{effective span} --- the range holding the central mass, swept across
the 95\%, 99\%, and 99.9\% bands --- and classifies the distribution's shape:
...
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Shape verdict} & \textbf{Meaning} \\
\midrule
\textsc{filled} & mass spread across the span; honest single-population support \\
\textsc{tail-inflated} & effective span notably below raw; a sparse tail is stretching it \\
\textsc{sparse-tail-artifact} & raw span dominated by tail; min--max M/N is void \\
\textsc{spiked} & one bin holds $\geq 25\%$ of mass; likely a single value or placeholder \\
\bottomrule
\end{tabular}
\caption{The four shape verdicts. Only \textsc{filled} support carries an honest register claim;
the others are reported with the effective span used in place of min--max.}
\end{table}

The run log prints the full 25-bin histogram of every lake alongside its verdict, because the
histogram is the evidence and the verdict is only its summary. Figure~\ref{fig:support-shapes}
is the survey-wide panel this produces.
...
\midrule
\texttt{python run\_pipeline\_period.py} & full run, all stages, figures included \\
\texttt{-{}-skip-figures} & all stages except figures \\
\texttt{-{}-support-only} & logify + support triage; no scan or census \\
\texttt{-{}-skip-census} & scan without the survey-wide census \\
\texttt{-{}-from scan} & resume from a named stage \\
\texttt{-{}-cap N} & sample cap override (default 2{,}000{,}000) \\
\bottomrule
\end{tabular}
\end{table}

Two standalone diagnostics run outside the pipeline:
\texttt{register\_stability.py <lake>} resamples a lake to test whether its winning register is
\emph{invariant} (a real lock) or \emph{wanders} (a binning artifact); and
\texttt{histogram\_view.py <lake>} prints one lake's support shape for troubleshooting.
\texttt{sidecar\_period.py}, run in a second terminal, renders live progress.

\section{Resource Adaptation: Turtle to Beast, Same Numbers}

A core commitment of this programme is that the work runs on the machine a person already owns. A
result that reproduces only on exotic hardware is not open. The period engine is therefore
resource-adaptive, and the adaptation changes \emph{speed only} --- never the numbers.
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Mechanism} & \textbf{On a potato} & \textbf{On a workstation} \\
\midrule
lake reads & streamed, one value at a time & streamed (RAM never a limit) \\
oversize lakes & reservoir-sampled to the cap, logged loudly & sampled or full via \texttt{-{}-cap} \\
resultant sum & NumPy, single core & CuPy on GPU when present and large \\
register scan / bootstrap & serial & thread-parallel across cores \\
\bottomrule
\end{tabular}
\caption{The engine detects cores and GPU at startup and prints what it adapted to. The circular
resultant was verified identical to six decimal places between the GPU/threaded path and pure
single-core NumPy: the turtle and the rabbit compute the same physics.}
\end{table}

Every oversize lake that is sampled records the fact in the run log with the exact command to
re-run it in full on adequate memory. Nothing is sampled silently.
...
\midrule
run log (tee'd, full stage detail + histograms) & \texttt{lakes1d\_period/logs/run\_period\_*.log} \\
receipt (fingerprint, resources, runtime, cap) & \texttt{lakes1d\_period/logs/run\_period\_*.receipt.txt} \\
portrait (per-lake support, shape, resolvable) & \texttt{lakes1d\_period/unified/period\_portrait.json} \\
heartbeat (live, reset each run) & \texttt{lakes1d\_period/unified/period\_heartbeat.json} \\
figures + index (when plugins present) & \texttt{reports1d\_period/output/run\_*/} \\
\bottomrule
\end{tabular}
\end{table}

Every run stamps a 160-character fingerprint --- the MD5 of the five files that fully determine
period output (\texttt{logify.py}, \texttt{periodogram.py}, \texttt{kish\_period\_io.py},
\texttt{logify.json}, \texttt{harmonic\_targets.json}). A change to the statistic, the field map, or
the register family shows as a single slot diff, exactly the trust mechanism that let Volume~12
trace a register shift to a one-line numerical patch.

\section{The Reporting Suite}

The figures stage auto-discovers every \texttt{report\_*.py} plugin in \texttt{reports1d\_period/},
builds a context dictionary from the run's artifacts, and calls each plugin's
\texttt{generate(context)} in sequence, writing an index of what was produced. The stage is a clean
no-op when no plugins are present, so a run never fails for lack of reports. The first plugin,
\texttt{report\_01\_support\_shapes.py}, renders the survey-wide support panel of
Figure~\ref{fig:support-shapes}.

\section{First-Light Result}

Run against the full survey --- eleven lakes, forty million records, on a twenty-core machine in
under ten minutes, with five oversize lakes sampled and flagged --- the engine returned a clean and
uncomfortable result, reproduced here exactly as the census reported it:

\begin{signalbox}
\textbf{No register is both resolved and locked above the look-elsewhere bar by any domain on
effective support.} Of eleven lakes, exactly one --- pulsar spin frequency --- shows \textsc{filled}
single-population support, and it locks no register: its winning register wanders under resampling,
the signature of a binning artifact rather than a lock. Every other lake is \textsc{spiked} or
\textsc{tail-inflated}, dominated by one band or a sparse tail.
\end{signalbox}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Title/support_shapes.png}
\caption{Support shape of every lake in the survey, coloured by verdict: green \textsc{filled},
amber \textsc{tail-inflated}, red \textsc{spiked}. Only pulsar spin frequency (top right) is honest
single-population support. The stitched ``universal'' composites are all spiked or tail-inflated ---
the visual signature of cross-population assembly that the cross-scale-stack chapter warns is not a
single lake. Nuclear half-life (\texttt{L\_q5\_decay}) shows its characteristic single lump with an
isolated placeholder spike near $\ln x \approx 58$. This one panel is the census result made
visible: no honest lake resolves and locks a register.}
\label{fig:support-shapes}
\end{figure}

This is not the engine failing. It is the engine doing precisely what it was built to do: refuse to
report a register the data cannot support, name every bad input rather than crash on it, and show its
evidence. The instrument is trustworthy. Whether the framework it tests will produce a survivable
register awaits lakes with genuinely wide, genuinely filled, single-population support --- the
subject of the standing pre-registrations.
...
% ==============================================================================
\chapter{The Contribution: An Instrument, Offered}
% ==============================================================================

\begin{center}
\textit{This volume ends not with a discovery but with an instrument. What follows is a plain
statement of what has been built, why it is useful independently of whether the KishLattice
hypothesis is true, and what ``learning the dials'' can and cannot mean without becoming the thing
this programme exists to avoid.}
\end{center}

\section{What Was Actually Built}

Across Volumes 12 and 13 the programme produced something separable from its own hypothesis: a
pair of reproducible engines for testing geometric-register structure in physical data, spanning
roughly forty-four orders of magnitude of scale through a single pipeline, with no per-domain
special-casing. The phase engine tests where structure sits on the grid; the period engine tests
whether it repeats with the lattice period at all, with unit invariance as a proved identity.

The engineering fact is worth stating on its own terms. One statistic, one field-resolution
contract, and data from nuclear timescales to cosmic distances flowing through it --- that breadth
is unusual, and it is real regardless of what the engines find.

\begin{signalbox}
What the engines find, so far, is null. The register census returns empty on effective support;
the one honest single-population lake (pulsar spin) wanders under resampling; the one clean
chemical modulus test finds $16/\pi$ matched or beaten by a majority of random moduli. \textbf{The
instrument spans forty-four orders of magnitude and reports honestly across all of them, and what
it reports is that the leading hypothesis does not yet clear the bar.}

\medskip
\noindent Both halves of that sentence are the contribution. The span is the achievement. The null
is the integrity. An engine that had confirmed its maker's hypothesis on its first honest run would
be \emph{less} credible, not more.
\end{signalbox}

\section{Why the Instrument Is a Contribution Even If the Hypothesis Is Wrong}

Most attempts to establish a unifying constant share a fatal structure: a bespoke analysis that
only its author can run, that tends to produce the answer the author wanted, and that no one can
independently check. The value of what is offered here is that it is the opposite of all three.
...
\toprule
\textbf{The common failure} & \textbf{What this engine does instead} \\
\midrule
Bespoke, author-only analysis & Runs on commodity hardware; streams and adapts, turtle to beast, same numbers \\
Produces the desired answer & Carries a resolution gate, a shape gate, an invariance test, a look-elsewhere threshold, and a modulus sweep that can reject the modulus \\
Cannot be independently checked & Fingerprinted per run; every result reproducible from public lakes and published scripts \\
Fits parameters to the data & Aborts, by name, when a required field is absent; simulates nothing \\
\bottomrule
\end{tabular}
\end{table}

\begin{signalbox}
\textbf{The engine can return null, and has.} That is the property that makes it a scientific
instrument rather than an advocacy device. A tool that cannot disappoint its author cannot inform
anyone.
\end{signalbox}

The deepest form of this contribution is temporal. Because the engine was built before any given
hypothesis is posed against it, it cannot be accused of being tuned to that hypothesis. It is
pre-registration by construction: a neutral instrument that predates the claims it will test.
Whoever proposes a better scalarization, a different modulus, or a mechanism that fixes the cell
scale can test it \emph{immediately}, against real public data, in an engine whose neutrality is
guaranteed by the fact that it came first.

\section{Learning the Dials Without Tuning Them}

The programme's structural choices --- a scalarization reference scale, a modulus, a container
value --- are not yet all fixed from first principles. It is tempting to say they can be
``learned'' by empirical testing. That statement is true only under a definition strict enough to
keep it from collapsing into its opposite, and the definition must be stated plainly, because a
reader cannot otherwise tell learning from tuning: both look, from outside, like someone adjusting
parameters until the data locks.

\begin{warningbox}
\textbf{A dial is \emph{learned}, not \emph{tuned}, if and only if its value is fixed by an argument
external to the fit, and filed before the data that would test it is seen.} $x_0$ set from an
independent physical reference scale and cited is learned; $x_0$ chosen because it makes a register
appear is tuned. A modulus derived from a geometric mode count is learned; a modulus selected
because it locks the most domains is tuned. The distinction is not in the parameter. It is in
\emph{when} and \emph{on what grounds} it was fixed.
\end{warningbox}

The engine enforces this distinction rather than trusting the operator to observe it. The period
engine carries no $x_0$ at all --- the parameter that could most easily be tuned was removed by
construction. The modulus is not asserted but swept against tens of thousands of alternatives, and
reported ordinary when it is ordinary. The resolution gate refuses a register the data cannot
resolve, so no dial can be turned to manufacture one. These are not courtesies; they are the
mechanisms that make ``learned'' a defensible word in this programme's mouth.

\begin{signalbox}
Under that discipline, learning the dials is a legitimate scientific activity, and it points
somewhere specific: if a reference scale, a modulus, or a cell scale can be fixed by independent
physics --- the bulk-to-boundary projection of Chapter~5 is the open problem most likely to fix
one --- and the engine, unchanged, then returns a register that resolves and locks and survives
resampling, that would be a validation earned rather than arranged. It has not happened. The
instrument is built so that if it ever does, no one --- including its authors --- will be able to
dismiss it as a dial turned to taste.
\end{signalbox}

\section{What Is Offered, and to Whom}

To a skeptic: here is an engine that tried to break its author's hypothesis and, on the honest
runs, succeeded. Run it yourself; the scripts and the lakes are public and the numbers are
reproducible on the machine you already own.

To a theorist with a better formula: here is a pre-built, neutral instrument in which your
hypothesis can be tested against real data across forty-four orders of magnitude the moment you
state it --- with a resolution gate, a null, and a falsifier already in place, so that a positive
result would carry weight your own bespoke analysis never could.

To the programme itself: here is the standard the rest of the work is now held to. The instrument
can say no. It has said no. The task ahead is not to make it say yes, but to determine --- by
fixing the dials from physics rather than from preference --- whether reality says yes when the
instrument is finally pointed at it correctly.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Title/KishLatticeEngineControl.png}
\caption{The discovery, if it is there, will not be able to hide from this engine.
Its absence, if that is the truth, will not be able to hide either.
That is the whole of what an honest instrument is for.}
\label{fig:KishLatticeEngineControl}
\end{figure}
