...
def main():
    ...
    if len(ek) == 2:
        ...
        # A register comparison is only meaningful if BOTH lakes actually LOCK.
        # A lake with z ~ 0 has no register to compare (its "peak" is noise).
        za_mean = results[a]["mean"]; za_std = results[a]["std"]
        zb_mean = results[b]["mean"]; zb_std = results[b]["std"]
        a_locks = (za_mean - za_std) >= 5.0
        b_locks = (zb_mean - zb_std) >= 5.0
        na = results[a].get("n_note",""); nb = results[b].get("n_note","")
        if not (a_locks and b_locks):
            print(f"\n   -> COMPARISON WITHHELD. A register comparison requires BOTH lakes to lock.")
            if not a_locks:
                print(f"      L_fermi_velocity does NOT lock: z = {za_mean:.2f} +/- {za_std:.2f}")
                print(f"      (near zero at every register; its 'peak' is sampling noise, not a lock).")
            if not b_locks:
                print(f"      L_emission_nist does NOT lock: z = {zb_mean:.2f} +/- {zb_std:.2f}")
            print(f"      This is NOT a 'different registers' finding. One or both lakes have no")
            print(f"      signal to compare. Likely cause: insufficient n to clear the chaos null.")
            print(f"      The electron question cannot be answered until both probes actually lock.")
